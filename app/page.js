...
export default function Page() {
  ...
  const handleFile = useCallback((e) => {
    ...
    reader.onload = (ev) => { setCsvData(parseCSV(ev.target.result)); setResults(null); setStatus("idle"); setErrorMsg(""); };
    reader.readAsText(file);
  }, []);

  const runEval = useCallback(async () => {
    if (!csvData) return;
    abortRef.current = false; setStatus("running"); setErrorMsg(""); setResults(null);
    ...
    const computeStats = (key) => {
      ...
      return { actionable, general, errors, actionable_converted, actionable_notConverted, general_converted, general_notConverted, matchesExpected, valid, alignmentRate: valid > 0 ? matchesExpected / valid : 0 };
    };
  ...
  const filteredRows = results?.evaluated?.filter(r => {
    ...
    if (detailFilter === "actionable_but_converted") return (r.p1Predicted === "ACTIONABLE" || r.p2Predicted === "ACTIONABLE") && r.isConverted === "Yes";
    if (detailFilter === "general_but_bounced") return (r.p1Predicted === "GENERAL" || r.p2Predicted === "GENERAL") && r.isConverted === "No";
    return true;
  }) || [];
          ...
          {[[prompt1, setPrompt1, prompt1Label, setPrompt1Label, C.accent], [prompt2, setPrompt2, prompt2Label, setPrompt2Label, C.purple]].map(([p, setP, lbl, setLbl, clr], idx) => (
            <Card key={idx}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                ...
                <input value={lbl} onChange={e => setLbl(e.target.value)} style={{ background: C.bg, color: clr, border: "1px solid " + C.border, borderRadius: 4, padding: "2px 8px", fontSize: 12, fontWeight: 600, width: 100, marginLeft: "auto" }} />
              </div>
